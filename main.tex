...
To gain comprehensive insights into students' perceptions of the newly introduced DAFeeD concept and to test the usability of the reference implementation, we employed a survey-based approach, visualized in Figure \ref{fig:Study-Design}.
\begin{figure}[htbp]
  \centering
...
At the beginning of the study, participants received a two-page participation instruction manual that included all necessary information. 
Study participants then tested the new feedback feature with a sample exercise using the Artemis platform in a controlled environment at the university.
%todo Felix: describe sample exercise
For the sample programming exercise, students needed to review code and had to improve the submission by fixing issues such as code duplication, missing methods, and implementation of wrong modifiers.
During the testing phase, participants followed DAFeeD's workflow, submitting their solution, requesting and reviewing the feedback, and refining their solution. 
Once the participants had a good understanding of the feedback process, participants were asked to complete a survey hosted on the community version of the open-source survey tool LimeSurvey\footnote{\url{https://www.limesurvey.org}}.
